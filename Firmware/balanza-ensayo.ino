/*
  ...
  CAMBIOS V2.0:
  - al encender, el actuador sube 6 s y queda arriba
  - despues de subir, hace un tare unico inicial
  - pesaje manual y automatico miden directo sin mover actuador
  - el pesaje normal ya no descuenta plateWeight
  - se mantiene la logica general, menus, UART y almacenamiento
*/
...
float runWeighRoutine(bool fromAutomaticMode) {
  ...
  Serial.println(F("WEIGH: MIDIENDO DIRECTO"));
  float measured = measureAverageFor2Seconds();
...
void printHelp() {
  ...
  Serial.println(F("WEIGH           -> ejecutar lectura manual"));
...
void setup() {
  ...
  Serial.println(F("SETUP: SUBIENDO ACTUADOR"));
  actuatorMoveUpFixed();

  delay(WAIT_AFTER_UP_MS);

  Serial.println(F("SETUP: TARE INICIAL"));
  doTare();
